// Gemini AI calls via Google AI Studio (no service account / Vertex AI required)
...
async function callGemini(modelId, systemPrompt, userMessage, retries = 2, timeoutMs = null) {
  ...
  const attempt = async () => {
    const req = model.generateContent(userMessage)
    let result
    if (timeoutMs) {
...
export async function writeRecommendation(extracted, topSimilarCase) {
  const modelId = process.env.GEMINI_MODEL_PRO || 'gemini-1.5-pro'
...
Write a brief triage recommendation for the intake worker.`
  ...
  try {
    return await callGemini(modelId, systemPrompt, message, 1, 10000)
  } catch {
