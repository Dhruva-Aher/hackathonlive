...
// Uses OAuth 2.0 UserRefreshClient (google-auth-library) + Vertex AI REST API directly.
// This bypasses @google-cloud/vertexai SDK auth which doesn't support authorized_user
// credentials in serverless environments.
//
// Env vars needed:
...
// Cache the auth client across invocations (warm Lambda reuse)
...
function getAuthClient() {
  ...
  return { client: _authClient, project }
}

async function getAccessToken() {
  const { client } = getAuthClient()
  const { token } = await client.getAccessToken()
  if (!token) throw new Error('Failed to obtain access token from OAuth refresh token')
  return token
}
...
async function callGemini(modelId, systemPrompt, userMessage, retries = 2, timeoutMs = null) {
  ...
  const attempt = async () => {
    ...
    if (timeoutMs) {
      ...
      res = await Promise.race([fetchCall, timeout])
    } else {
      res = await fetchCall
    }
...
export async function writeRecommendation(extracted, topSimilarCase) {
  const modelId = process.env.GEMINI_MODEL_PRO || 'gemini-2.0-flash-001'
