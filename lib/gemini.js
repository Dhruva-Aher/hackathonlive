...
function getConfig() {
  ...
  return { project, clientId, clientSecret, refreshToken }
}

// Exchange refresh token for access token via Google token endpoint directly
...
export async function extractCaseFacts(rawText) {
  ...
  const modelId   = process.env.GEMINI_MODEL_FLASH || 'gemini-2.0-flash'
  try {
...
export async function writeRecommendation(extracted, topSimilarCase) {
  const modelId = process.env.GEMINI_MODEL_PRO || 'gemini-2.0-flash'
