...
const EXTRACTION_SYSTEM_PROMPT = `You are a legal intake analyst for a legal aid clinic. Extract structured information from intake documents. Return ONLY valid JSON with no explanation, no markdown fences, no preamble. If a field cannot be determined, use null for strings/numbers or false for booleans.`
...
Write a brief triage recommendation for the intake worker.`
  ...
  try {
    return await callGemini(modelId, systemPrompt, userMessage, 1, 8000)
  } catch {
